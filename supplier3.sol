...
contract Supplier {
    ...
    receive() external payable {
         // get balance after the deposit is returned
        uint excess = address(r).balance;
        if (excess > 0){
               r.retrieve_resource();
        }
    ...
    function transfer_to_rental(uint money) public payable{
        (bool success, ) = address(r).call.value(money)("");
...
contract Rental {
    ...
    function retrieve_resource() external payable {
        ...
        // RETURN DEPOSIT HERE
        // Make resource_available to true before returning the deposit
        resource_available = true;
        (bool success, ) = address(resource_owner).call.value(deposit)("");
        require(success, "Fail to return deposit");
    }
    function getContractBalance() public view returns (uint256) {
    return address(this).balance;
}
